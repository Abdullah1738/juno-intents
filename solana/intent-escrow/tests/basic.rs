...
use juno_intents_checkpoint_registry::{checkpoint_pda as crp_checkpoint_pda, config_pda as crp_config_pda, CrpInstruction};
use juno_intents_intent_escrow::{
    config_pda as iep_config_pda,
    fill_pda as iep_fill_pda,
    intent_pda as iep_intent_pda,
    spent_receipt_pda as iep_spent_pda,
    vault_pda as iep_vault_pda,
    IepFillV1, IepInstruction, IepIntentV1,
};
...
async fn settle_transfers_net_and_fee_and_marks_spent() {
    ...
    ixs.push(mint_to_ix(&mint.pubkey(), &solver_ta.pubkey(), &payer.pubkey(), 1_000_000));
