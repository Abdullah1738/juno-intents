...
const OPERATOR_REGISTRY_OPERATOR_SEED: &[u8] = b"operator";
const OPERATOR_REGISTRY_OPERATOR_RECORD_LEN_V1: usize = 1 + 32 + 32 + 32 + 1;
...
pub const CONFIG_LEN_V2: usize = CONFIG_LEN_V1 + 32;
...
pub enum CrpInstruction {
    ...
    InitializeV2 {
        ...
        conflict_threshold: u8,
        finalization_delay_slots: u64,
        operator_registry_program: Pubkey,
        operators: Vec<Pubkey>,
    },
}
...
pub struct CrpConfigV2 {
    ...
    pub conflict_threshold: u8,
    pub finalization_delay_slots: u64,
    pub operator_registry_program: Pubkey,
    pub operator_count: u8,
    pub operators: [Pubkey; MAX_OPERATORS],
    pub paused: bool,
}
...
fn process_initialize_v2(
    ...
    if operator_registry_program == Pubkey::default() {
...
fn process_set_operators(program_id: &Pubkey, accounts: &[AccountInfo], operators: Vec<Pubkey>) -> ProgramResult {
    ...
    let version = config_ai.data.borrow().get(0).copied().unwrap_or(0);
    match version {
        CONFIG_VERSION_V1 => {
            let mut cfg = CrpConfigV1::try_from_slice(&config_ai.data.borrow())
                .map_err(|_| ProgramError::from(CrpError::InvalidAccountData))?;
            validate_config_pda_v1(program_id, config_ai, &cfg)?;
        ...
        CONFIG_VERSION_V2 => {
            let mut cfg = CrpConfigV2::try_from_slice(&config_ai.data.borrow())
                .map_err(|_| ProgramError::from(CrpError::InvalidAccountData))?;
            validate_config_pda_v2(program_id, config_ai, &cfg)?;
            if cfg.admin != *admin.key {
                return Err(CrpError::Unauthorized.into());
            }
        ...
        _ => Err(CrpError::InvalidAccountData.into()),
    }
}
...
fn process_submit_observation(
    ...
    // 5. operator_record (PDA, required for config v2)
    let mut iter = accounts.iter();
    let payer = next_account_info(&mut iter)?;
    let config_ai = next_account_info(&mut iter)?;
    ...
    match version {
        CONFIG_VERSION_V1 => {
            ...
            let message =
                observation_signing_bytes_v1(&cfg.deployment_id, height, &block_hash, &orchard_root, &prev_hash);
            let signer = verify_single_ed25519_signature(instructions_sysvar, &message)?;
            if operator_bit_v1(&cfg, &signer).is_none() {
                return Err(CrpError::InvalidOperatorSignature.into());
            }

            handle_checkpoint_create_or_verify(
                program_id,
                payer,
                config_ai,
                checkpoint_ai,
        ...
        CONFIG_VERSION_V2 => {
            let cfg = CrpConfigV2::try_from_slice(&config_ai.data.borrow())
                .map_err(|_| ProgramError::from(CrpError::InvalidAccountData))?;
            validate_config_pda_v2(program_id, config_ai, &cfg)?;
            if cfg.paused {
                return Err(CrpError::Paused.into());
            ...
            let message =
                observation_signing_bytes_v1(&cfg.deployment_id, height, &block_hash, &orchard_root, &prev_hash);
            let signer = verify_single_ed25519_signature(instructions_sysvar, &message)?;
            if operator_bit_v2(&cfg, &signer).is_none() {
                return Err(CrpError::InvalidOperatorSignature.into());
            }

            let operator_record_ai = next_account_info(&mut iter)?;
            verify_operator_record_v1(&cfg, operator_record_ai, &signer)?;
...
#[inline(never)]
fn handle_checkpoint_create_or_verify<'a>(
    program_id: &Pubkey,
    payer: &AccountInfo<'a>,
    config_ai: &AccountInfo<'a>,
    checkpoint_ai: &AccountInfo<'a>,
...
fn process_finalize_checkpoint(program_id: &Pubkey, accounts: &[AccountInfo], sig_count: u8) -> ProgramResult {
    ...
    let checkpoint_ai = &accounts[2];
    let height_ai = &accounts[3];
    let system_program = &accounts[4];
    let instructions_sysvar = &accounts[5];
    let operator_records = accounts.get(6..).unwrap_or(&[]);
    ...
    let version = config_ai.data.borrow().get(0).copied().unwrap_or(0);
    match version {
        CONFIG_VERSION_V1 => finalize_checkpoint_v1(
            program_id,
            payer,
            config_ai,
            checkpoint_ai,
        ...
        CONFIG_VERSION_V2 => finalize_checkpoint_v2(
            program_id,
            payer,
            config_ai,
            checkpoint_ai,
...
#[inline(never)]
fn finalize_checkpoint_v1<'a>(
    program_id: &Pubkey,
    payer: &AccountInfo<'a>,
    config_ai: &AccountInfo<'a>,
    checkpoint_ai: &AccountInfo<'a>,
    height_ai: &AccountInfo<'a>,
    system_program: &AccountInfo<'a>,
...
fn finalize_checkpoint_v2<'a>(
    ...
    system_program: &AccountInfo<'a>,
    instructions_sysvar: &AccountInfo<'a>,
    operator_records: &[AccountInfo<'a>],
    sig_count: u8,
) -> ProgramResult {
    let mut cfg = CrpConfigV2::try_from_slice(&config_ai.data.borrow())
        .map_err(|_| ProgramError::from(CrpError::InvalidAccountData))?;
    ...
    let (signer_mask, signers) = verify_ed25519_signatures_v2(instructions_sysvar, &cfg, &message, sig_count)?;
    if signer_mask.count_ones() < cfg.threshold as u32 {
        return Err(CrpError::InsufficientSignatures.into());
    }

    // Enforce attested operator identity for each signature.
    if operator_records.len() < signers.len() {
        return Err(ProgramError::NotEnoughAccountKeys);
    }
    for (i, signer) in signers.iter().enumerate() {
        verify_operator_record_v1(&cfg, &operator_records[i], signer)?;
    }
...
fn process_mark_conflict(program_id: &Pubkey, accounts: &[AccountInfo], sig_count: u8) -> ProgramResult {
    ...
    match version {
        CONFIG_VERSION_V1 => {
            ...
            let msg_a =
                observation_signing_bytes_v1(&cfg.deployment_id, a.height, &a.block_hash, &a.orchard_root, &a.prev_hash);
            let msg_b =
                observation_signing_bytes_v1(&cfg.deployment_id, b.height, &b.block_hash, &b.orchard_root, &b.prev_hash);
            let (mask_a, mask_b) =
        ...
        CONFIG_VERSION_V2 => {
            // Accounts after `instructions_sysvar`:
            // - operator_record (PDA) x sig_count (must match signature order)
            ...
            let msg_b =
                observation_signing_bytes_v1(&cfg.deployment_id, b.height, &b.block_hash, &b.orchard_root, &b.prev_hash);
            let (mask_a, mask_b, signers) =
                verify_ed25519_conflict_signatures_v2(instructions_sysvar, &cfg, &msg_a, &msg_b, sig_count)?;
            if mask_a == 0 || mask_b == 0 {
                return Err(CrpError::InsufficientSignatures.into());
            }
            let total = (mask_a | mask_b).count_ones() as u8;
            ...
            for signer in signers.iter() {
                let op_rec_ai = next_account_info(&mut iter)?;
                verify_operator_record_v1(&cfg, op_rec_ai, signer)?;
            }
...
fn validate_config_pda_v1(program_id: &Pubkey, config_ai: &AccountInfo, cfg: &CrpConfigV1) -> ProgramResult {
    validate_config_pda(program_id, config_ai, cfg)
}

fn validate_config_pda_v2(program_id: &Pubkey, config_ai: &AccountInfo, cfg: &CrpConfigV2) -> ProgramResult {
...
fn operator_bit_v1(cfg: &CrpConfigV1, pk: &Pubkey) -> Option<u32> {
    operator_bit(cfg, pk)
}

fn operator_bit_v2(cfg: &CrpConfigV2, pk: &Pubkey) -> Option<u32> {
...
fn verify_ed25519_signatures_v2(
    instructions_sysvar: &AccountInfo,
    cfg: &CrpConfigV2,
    expected_message: &[u8; OBS_SIGNING_BYTES_LEN],
    sig_count: u8,
) -> Result<(u32, Vec<Pubkey>), ProgramError> {
    let cur = load_current_index_checked(instructions_sysvar)? as usize;
    ...
    for i in 0..count {
        ...
        let bit = operator_bit_v2(cfg, &pk).ok_or(CrpError::InvalidOperatorSignature)?;
        if (mask & bit) != 0 {
...
fn verify_ed25519_conflict_signatures_v2(
    instructions_sysvar: &AccountInfo,
    cfg: &CrpConfigV2,
    msg_a: &[u8; OBS_SIGNING_BYTES_LEN],
    msg_b: &[u8; OBS_SIGNING_BYTES_LEN],
    sig_count: u8,
) -> Result<(u32, u32, Vec<Pubkey>), ProgramError> {
    let cur = load_current_index_checked(instructions_sysvar)? as usize;
    let count = sig_count as usize;
    if cur < count {
        return Err(CrpError::InvalidEd25519Instruction.into());
...
fn verify_operator_record_v1(cfg: &CrpConfigV2, operator_record_ai: &AccountInfo, signer: &Pubkey) -> ProgramResult {
    if operator_record_ai.owner != &cfg.operator_registry_program {
        return Err(CrpError::OperatorNotRegistered.into());
    }
    if operator_record_ai.data_len() != OPERATOR_REGISTRY_OPERATOR_RECORD_LEN_V1 {
        return Err(CrpError::OperatorNotRegistered.into());
    }

    let (expected_record, _bump) = Pubkey::find_program_address(
        &[
            OPERATOR_REGISTRY_OPERATOR_SEED,
            cfg.deployment_id.as_ref(),
            signer.as_ref(),
        ],
        &cfg.operator_registry_program,
    );
    if expected_record != *operator_record_ai.key {
        return Err(CrpError::OperatorNotRegistered.into());
    }

    let rec = OrpOperatorRecordV1::try_from_slice(&operator_record_ai.data.borrow())
        .map_err(|_| ProgramError::from(CrpError::OperatorNotRegistered))?;
    if rec.version != 1
        || rec.deployment_id != cfg.deployment_id
        || rec.operator_pubkey != *signer
        || !rec.enabled
    {
        return Err(CrpError::OperatorNotRegistered.into());
    }
