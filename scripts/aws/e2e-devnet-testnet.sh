...
solana_version_num=solana_version[1:] if solana_version.startswith("v") else solana_version
  ...
  "sudo yum -y install clang llvm-devel || true",
  "if ! /usr/local/bin/protoc --version 2>/dev/null | grep -q 'libprotoc 3.20.3'; then sudo rm -rf /opt/protoc-3.20.3 /tmp/protoc-3.20.3.zip && curl -sSfL https://github.com/protocolbuffers/protobuf/releases/download/v3.20.3/protoc-3.20.3-linux-x86_64.zip -o /tmp/protoc-3.20.3.zip && sudo mkdir -p /opt/protoc-3.20.3 && sudo unzip -o /tmp/protoc-3.20.3.zip -d /opt/protoc-3.20.3 >/dev/null && sudo ln -sf /opt/protoc-3.20.3/bin/protoc /usr/local/bin/protoc; fi",
  ...
  'export PATH=\"/usr/local/cuda/bin:/usr/local/cuda-12.9/bin:/usr/local/go/bin:$HOME/.cargo/bin:/opt/solana/solana-release/bin:/usr/local/bin:$PATH\"',
  "export PROTOC=/usr/local/bin/protoc",
  "export LIBCLANG_PATH=/lib64",
  "if [ -x /usr/local/cuda/bin/nvcc ]; then export NVCC=/usr/local/cuda/bin/nvcc; elif [ -x /usr/local/cuda-12.9/bin/nvcc ]; then export NVCC=/usr/local/cuda-12.9/bin/nvcc; fi",
  "export NVCC_APPEND_FLAGS=-DNDEBUG",
  "ccap=\"$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader 2>/dev/null | head -n 1 | tr -d ' \\t\\r\\n' || true)\"; if echo \"${ccap}\" | grep -Eq '^[0-9]+\\.[0-9]+$'; then major=\"${ccap%.*}\"; minor=\"${ccap#*.}\"; export NVCC_PREPEND_FLAGS=\"-arch=sm_${major}${minor}\"; else export NVCC_PREPEND_FLAGS=-arch=native; fi",
  "if command -v gcc10-gcc >/dev/null; then export CC=gcc10-gcc CXX=gcc10-g++; fi",
  "export JUNO_RISC0_USE_DOCKER=1",
  "go version",
  "if ! command -v cargo >/dev/null; then curl -sSf https://sh.rustup.rs | sh -s -- -y; fi",
  'export PATH=\"$HOME/.cargo/bin:/usr/local/cuda/bin:/usr/local/cuda-12.9/bin:/usr/local/go/bin:/opt/solana/solana-release/bin:/usr/local/bin:$PATH\"',
...
cmds=[
  ...
  "ccap=\"$(nvidia-smi --query-gpu=compute_cap --format=csv,noheader 2>/dev/null | head -n 1 | tr -d ' \\t\\r\\n' || true)\"; if echo \"${ccap}\" | grep -Eq '^[0-9]+\\.[0-9]+$'; then major=\"${ccap%.*}\"; minor=\"${ccap#*.}\"; export NVCC_PREPEND_FLAGS=\"-arch=sm_${major}${minor}\"; else export NVCC_PREPEND_FLAGS=-arch=native; fi",
  "if command -v gcc10-gcc >/dev/null; then export CC=gcc10-gcc CXX=gcc10-g++; fi",
  "export CARGO_TARGET_DIR=/var/lib/docker/cargo-target",
