...
cmds=[
  ...
  "sudo apt-get update",
  "sudo apt-get install -y --no-install-recommends git ca-certificates curl jq unzip protobuf-compiler bzip2",
  "if ! command -v docker >/dev/null; then sudo apt-get install -y --no-install-recommends docker.io; fi",
  ...
  "if ! command -v nitro-cli >/dev/null; then sudo apt-get install -y --no-install-recommends aws-nitro-enclaves-cli || sudo apt-get install -y --no-install-recommends nitro-enclaves-cli || true; fi",
