...
scripts/junocash/testnet/up.sh

jcli() {
  scripts/junocash/testnet/cli.sh "$@"
}
...
shield_limit="${JUNO_TESTNET_SHIELD_LIMIT:-1}"
...
txid=""
wait_secs="${JUNO_TESTNET_SHIELD_WAIT_SECS:-1800}"
...
if [[ -z "${txid}" ]]; then
  ...
  docker logs --tail 200 "${JUNO_TESTNET_CONTAINER_NAME_A:-juno-testnet-a}" >&2 || true
  exit 1
fi
